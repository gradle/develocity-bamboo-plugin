...
public class GradleEnterprisePreBuildAction extends AbstractBuildTask implements CustomPreBuildAction {
    ...
    public BuildContext call() {
        for (BuildScanInjector<? extends BuildToolConfiguration> injector : injectors) {
            try {
                ...
                LOGGER.error("Gradle Enterprise {} auto-injection failed", injector.buildTool().displayName(), e);
    ...
    private void addErrorToBuildLog(BuildScanInjector injector, Exception ex) {
        try {
            String message = String.format("Gradle Enterprise %s auto-injection failed", injector.buildTool().displayName());
