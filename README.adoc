...
As of version 1.3.0, the plugin contains renaming from Gradle Enterprise to Develocity. However, it was still available under the same maven coordinates.
Version 2.0.0 updates that to `com.gradle.develocity:develocity-bamboo-plugin`. It also contains several internal changes that make it backwards incompatible with 1.3.0.
...
To upgrade from 1.3.0 to 2.0.0, one needs to do the following:

1. Take note of the current configuration settings as after upgrade is done, the plugin would need to be configured again.
2. Uninstall the app via `Manage Apps` section in Bamboo Administration page.
...
The plugin can be manually installed as mentioned in https://confluence.atlassian.com/bamboo/installing-a-plugin-289277265.html[Uploading your own plugin] section. The released JARs can be found in the Releases.
