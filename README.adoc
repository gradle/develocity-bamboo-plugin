...
The plugin exposes the links to the created build scans in the Bamboo UI. It connects Gradle and Maven builds to an existing Gradle Enterprise instance such that a build scan is published each time a build is run from Bamboo.
