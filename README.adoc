...
You’d need to specify your Gradle Enterprise server URL, select “Allow untrusted server” if applicable, check the "Enables Gradle Enterprise Maven extension auto-injection" checkbox and, if required, specify the name of the Bamboo shared credential that holds the access key for authenticating with the Gradle Enterprise server.
...
|Next version           | 1.17.4                                    | 1.12.1                                           | 2022.3
